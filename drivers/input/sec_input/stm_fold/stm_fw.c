...
#define STM_TS_FILE_SIGNATURE_TS2C	0xCEFABEBA
...
struct stm_ts_header_ts2c {	// Total 36 bytes
	u32	fw_crc;				// 0
...
#define CX_ADDR_START_0 0x00007000
#define CONFIG_ADDR_START_0 0x00007C00
...
int stm_ts_parse_bin_file(struct stm_ts_data *ts, const u8 *ubin_data, int ubin_size, struct firmware_file *fw_data)
{
	...
	input_info(true, &ts->client->dev, "%s: ubin_size %d\n", __func__, ubin_size);
	...
	while (index < ubin_size) {
		...
		if (u16_temp == FINGERTIP_FW_CODE) {
			...
			fw_data->fw_code_data = (u8 *)kmalloc(fw_data->fw_code_size * sizeof(u8), GFP_KERNEL);
			if (fw_data->fw_code_data == NULL) {
				input_err(true, &ts->client->dev, "%s: Error allocating memory\n", __func__);
				return -1;
			}
			...
			fw_data->sections[sec_index].sec_data = (u8 *)kmalloc(fw_data->sections[sec_index].sec_size * sizeof(u8), GFP_KERNEL);
...
int stm_ts_flash_erase(struct stm_ts_data *ts, int flash_pages)
{
	...
	rc = ts->stm_ts_i2c_write(ts, &reg[0], 5 + mask_cnt, NULL, 0);
	if (rc < 0)
		return rc;
	...
	reg[3] = 0x00;
	reg[4] = 0x6E;
	rc = ts->stm_ts_i2c_read(ts, &reg[0], 5, &data, 1);
	if (rc < 0)
		return rc;
...
int stm_ts_i2c_write_fw_reg(struct stm_ts_data *ts, u16 address, u8 *data, uint32_t length)
{
	int rc = 0;
	u8 reg[128 + 2] = {0};
	...
	rc = ts->stm_ts_i2c_write(ts, &reg[0], length + 2, NULL, 0);
	if (rc < 0)
 ...
 * @return OK if success or an error code which specify the type of error
 */
int stm_ts_i2c_read_fw_reg(struct stm_ts_data *ts, u16 address, u8 *read_data, uint32_t read_length)
{
	int rc = 0;
	u8 reg[2] = {0};
	...
	rc = ts->stm_ts_i2c_read(ts, &reg[0], 2, &read_data[0], read_length);
	if (rc < 0)
		input_err(true, &ts->client->dev, "%s: ERROR\n", __func__);
...
int stm_ts_fw_request(struct stm_ts_data *ts, u16 address, u8 bit_to_set, int time_to_wait)
{
	...
	rc = stm_ts_i2c_read_fw_reg(ts, address, &data, 1);
	if (rc < 0) {
		input_err(true, &ts->client->dev, "%s: ERROR\n", __func__);
		return rc;
	}
	...
	rc = stm_ts_i2c_write_fw_reg(ts, address, &data, 1);
	if (rc < 0) {
		input_err(true, &ts->client->dev, "%s: ERROR\n", __func__);
		return rc;
	}
 ...
 * @return OK if success or an error code which specify the type of error
 */
int stm_ts_i2c_write_hdm(struct stm_ts_data *ts, u16 address, u8 *data, int length)
{
	int rc = 0;
	u8 *reg;
...
int stm_ts_flash_section_burn(struct stm_ts_data *ts, struct firmware_file fw, fw_section_t section)
{
	int rc = 0;
	int i = 0;
	...
	for (i = 0; i < fw.num_sections; i++) {
		if (fw.sections[i].sec_id == section) {
			rc = stm_ts_i2c_write_hdm(ts, FRAME_BUFFER_ADDR, fw.sections[i].sec_data, fw.sections[i].sec_size);
			if (rc < 0)
				return -1;
...
static int stm_ts_fw_burn_ts2c(struct stm_ts_data *ts, const u8 *fw_data, int fw_size)
{
	...
	if (fw.fw_code_size > 0) {
		...
		input_info(true, &ts->client->dev, "%s: Section System Errors: reg section: %02X, ms_section: %02X, ss_section: %02X\n",
				   __func__, (data[0] & STM_TS_REG_CRC_MASK), (data[1] & STM_TS_MS_CRC_MASK),
				   (data[1] & STM_TS_SS_CRC_MASK));

		input_info(true, &ts->client->dev, "%s: System Crc Errors: misc: %02X, ioff: %02X, pure_raw_ms: %02X\n",
				   __func__, (data[0] & STM_TS_REG_MISC_MASK), (data[2] & STM_TS_IOFF_CRC_MASK),
				   (data[3] & STM_TS_RAWMS_CRC_MASK));
	}
...
static const int stm_ts_fw_updater(struct stm_ts_data *ts, const u8 *fw_data, int fw_size)
{
	...
	if (ts->chip_id == 0x523601) {
		stm_ts_u8_to_u32_be(&fw_data[16], &fw_main_version_ts2c);
		input_info(true, &ts->client->dev,
				   "%s: Starting firmware update : 0x%04X\n", __func__,
				   fw_main_version_ts2c);
	} else {
	...
	while (1) {
		...
		if (retval >= 0) {
			...
			if (ts->chip_id == 0x523601) {
				...
				break;
			} else if (fw_main_version == ts->fw_main_version_of_ic) {
				input_info(true, &ts->client->dev,
...
int stm_ts_fw_update_on_probe(struct stm_ts_data *ts)
{
	...
	const struct stm_ts_header *header;
	const struct stm_ts_header_ts2c *header_ts2c;
	...
	if (ts->chip_id == 0x523601) {
		header_ts2c = (struct stm_ts_header_ts2c *)fw_entry->data;
		...
		ts->project_id_of_bin = (u8)(header_ts2c->project_id);
		ts->ic_name_of_bin = (u8)(header_ts2c->ic_name);
		ts->module_version_of_bin = (u8)(header_ts2c->module_ver);
...
static int stm_ts_load_fw(struct stm_ts_data *ts, int update_type)
{
	...
	if (ts->chip_id == 0x523601) {
		header_ts2c = (struct stm_ts_header_ts2c *)fw_entry->data;
		if (header_ts2c->header_signature != STM_TS_FILE_SIGNATURE_TS2C) {
			error = -EIO;
			input_err(true, &ts->client->dev,
					"%s: File type is not match with stm_ts64 file. [%8x]\n",
					__func__, header_ts2c->header_signature);
			goto done;
		}
		...
		input_info(true, &ts->client->dev,
			...
			(u16)header_ts2c->fw_ver, (u16)header_ts2c->ext_release_ver,
			header_ts2c->ic_name, header_ts2c->project_id, header_ts2c->module_ver);
	} else {
		header = (struct stm_ts_header *)fw_entry->data;
	...
	if (is_fw_signed) {
		...
		} else if (update_type == TSP_SPU) {
			if (ts->chip_id == 0x523601) {
				if (ts->ic_name_of_ic == header_ts2c->ic_name
					&& ts->project_id_of_ic == header_ts2c->project_id
					&& ts->module_version_of_ic == header_ts2c->module_ver) {
					if (ts->fw_version_of_ic >= header_ts2c->fw_ver) {
				...
				if (ts->ic_name_of_ic == header->ic_name
		...
		} else if (update_type == TSP_SDCARD) {
			if (ts->chip_id == 0x523601) {
				if (ts->ic_name_of_ic == header_ts2c->ic_name)
